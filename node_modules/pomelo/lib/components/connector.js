...
var desTool = require('../cryptTool/desServer.js');
...
var Component = function(app, opts) {
  ...
  this.keys = {};
  this.blacklist = [];
    this.useSelfCrypt = opts.useSelfCrypt
...
var bindEvents = function(self, socket) {
  ...
  socket.on('message', function(msg) {
      if(session.get('needCheck') && !session.get('isValidAlready')){
          self.app.logger.error('invalid client msg: ' + socket.remoteAddress.ip);
          return;
      }

      if(self.useSelfCrypt){
          msg.body = desTool.decipher(socket.__desSecret, msg.body.toString());
      }
    var dmsg = msg;
    if(self.decode) {
